...
router.get('/', requireAuth, async (req, res, next) => {
  try {
  const [solved, completedTopics, aptitudeAttempts, interviews, mocks, quests, dueSkillRevisions] = await Promise.all([
  ...
  res.json({ success: true, data: {
    stats: { solved, streak: 0, mockTests: mocks + savedMocks, skillsCompleted: completedTopics, aptitude, advancedAptitude, interviews, dailyChallengesCompleted, dueSkillRevisions },
